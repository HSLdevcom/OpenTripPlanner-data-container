...
const { dataDir, storageDir, configsDir } = require('../config');
...
function createAndProcessRouterConfig(router) {
  ...
  const configName = `${configsDir}/${router.id}/router-config.json`;
...
function prepareRouterData(router) {
  ...
  stream.push(
    createFile(router, 'otp-config.json', `${configsDir}/${router.id}`),
  );
